...
export const levelScore=node=>node.boss??operationScore(node.theme)+node.difficulty+node.speed+node.strictness+(node.questions??5)-9;
export function tuneLevel(node,change){
  ...
  node.questions??=5;
  const target=Math.max(0,levelScore(node)+change);
...
export function makeMap(seed,offset=0,mode=1){
  const rng=random(seed+offset),progress=random(seed^0x51a7),themes=["+","−","×","+−","+×","−×","+−×"];
  const increment=()=>mode?mode+Math.floor(progress()*2):1;
  let rating=[0,5,10][mode];for(let i=0;i<offset;i++)rating+=increment();
  let spine=2;
  return Array.from({length:10},(_,row)=>{
    const base=rating;rating+=increment();
    spine=Math.max(0,Math.min(4,spine+Math.floor(rng()*3)-1));
    return Array.from({length:5},(_,lane)=>{
      const stage=offset+row,challenge=stage>0&&rng()<.15,target=base+4+(challenge?2:0),available=themes.filter(t=>operationScore(t)+3<=target);
      ...
      const color=lane===spine?stage%7:Math.floor(rng()*7),pieces=[color];
